...
class Chess(pyglet.window.Window):
    ...
    def __init__(self):
        ...
        super(Chess, self).__init__(600, 600,
                                    ...
                                    vsync=False)  # FPS
        self.wKing = King(4, 0)  # Placement is made from right to left and from bottom to top
        self.bKing = King(4, 7, False)  # Type is False is piece is black
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        if self.promotion:  # If there is a promotion
            if button == mouse.LEFT:
                if 225 < y < 300:
                    ...
                    if 131.25 < x < 206.25:
                        ...
                        self.board[self.promoPawn[0]][self.promoPawn[1]] = Bishop(self.promoPawn[1],
                                                                                  self.promoPawn[0], not self.move)
                    elif 393.75 < x < 468.75:
                        self.board[self.promoPawn[0]][self.promoPawn[1]] = Knight(self.promoPawn[1],
                                                                                  self.promoPawn[0], not self.move)
                self.promoPawn = (-1, -1)
                ...
                if not self.move:  # If it's black's turn
                    if self.bKing.NoValidMoves(self.board) and not self.bKing.InCheck(
                    ...
                    if self.wKing.NoValidMoves(self.board) and not self.wKing.InCheck(
            ...
            if button == mouse.LEFT:
                boardX = x // 75
                boardY = y // 75
                if self.currentPos[0] < 0 and self.currentPos[
                    1] < 0:  # Goes inside because it's initialized with -1, -1
                    if self.board[boardY][boardX] is not None and self.move == self.board[boardY][
                        boardX].white:  # If there's a click from your side
                        ...
                        if self.move:  # If white
                            ValidMoves = self.board[boardY][boardX].GetValidMoves(self.board,
                                                                                  self.wKing)  # Put the white's valid move inside the variable
                        else:  # If black
                            ValidMoves = self.board[boardY][boardX].GetValidMoves(self.board,
                                                                                  self.bKing)  # Put the blacks's valid move inside the variable
                ...
                elif self.board[boardY][boardX] is not None and self.move == self.board[boardY][
                    boardX].white:  # If you have a piece selected and you wanna select another one
                    ...
                    if self.validsprites[boardY][boardX].visible:  # If possible moves visible
                        ...
                        if type(self.board[self.currentPos[0]][self.currentPos[1]]) is Pawn and (
                                boardY == 0 or boardY == 7):  # Check if there's a pawn at top or bottom
